...
app.get('/problem/:id/export', async (req, res) => {
  try {
    ...
    if (!await problem.isAllowedEditBy(res.locals.user)) {
      if (!req.query.token || req.query.token !== syzoj.config.default_token) {
        throw new ErrorMessage('您没有权限进行此操作。');
      }
    }
...
async function setPublic(req, res, is_public) {
  try {
    ...
    problem.publicizer_id = (is_public ? res.locals.user.id: null);
...
app.post('/problem/:id/submit', app.multer.fields([{ name: 'answer', maxCount: 1 }]), async (req, res) => {
  try {
    ...
    if (problem.type === 'submit-answer') {
      ...
      // The same submission within 10 seconds is considered a duplicate submission.
      let now = parseInt((new Date()).getTime() / 1000);
      let state = await problem.getJudgeState(res.locals.user, false);
      if (state && state.code === file.md5 && now - state.submit_time <= 10) {
        dep = true;
        judge_state = state;
      } else {
      ...
      let now = parseInt((new Date()).getTime() / 1000);
      let state = await problem.getJudgeState(res.locals.user, false);
      if (state && state.code === code && now - state.submit_time <= 10) {
        dep = true;
        judge_state = state;
      } else {
        judge_state = await JudgeState.create({
    ...
    if (class_id) {
      ...
      judge_state.type = 2;
      judge_state.type_info = course_id;
    ...
    // Skip judge duplicate submission.
    if (!dup) {
      try {
...
app.get('/problem/:id/download/additional_file', async (req, res) => {
  try {
    ...
    if (course_id) {
      ...
      if (!course) throw new ErrorMessage('无此课程。');
      const isSupervisior = await course.isSupervisior(res.locals.user);
      if (!isSupervisior) throw new ErrorMessage('您没有权限进行此操作。');
      if (!await course.hasProblem(problem)) throw new ErrorMessage('无此题目。');
    } else if (contest_id) {
